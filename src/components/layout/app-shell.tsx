...
export function AppShell({ profile, children }: { profile: Profile; children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <aside className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white/95 px-2 py-2 backdrop-blur md:inset-y-0 md:left-0 md:right-auto md:w-64 md:border-r md:border-t-0 md:p-4">
        ...
        <nav className="grid grid-cols-4 gap-1 md:block md:space-y-1">
          {nav.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className="flex flex-col items-center gap-1 rounded-2xl px-2 py-2 text-xs font-medium text-slate-600 hover:bg-emerald-50 hover:text-emerald-700 md:flex-row md:px-3 md:text-sm"
              >
          ...
          <form action={logoutAction}>
            <Button variant="ghost" size="sm" className="h-auto w-full flex-col md:flex-row">
